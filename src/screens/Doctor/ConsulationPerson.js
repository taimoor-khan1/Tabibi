...
import {getConsultationList, cancelAppointment} from '../../store/actions'

const ConsulationPerson = ({navigation, id}) => {
  //REDUX STORE
  ...
  useEffect(() => {
    if (consultaionList?.length > 0) {
      ...
      const patientExist =
        patients && patients.filter(text => keys.includes(text.patientId))
      const loveOneExist = {}
      loveOnes &&
        loveOnes.map(item => {
...
export default ConsulationPerson
