...
const Chat = ({navigation, route}) => {
  ...
  const [user_id] = useState(profile?.userId.replace(/-/g, ''))
  const [user_name] = useState(profile?.name)
  const [user_image] = useState(profile?.avatar)
  ...
  // Action for firebase database ON/OFF
  useEffect(() => {
  ...
  return (
    <View style={[styles.mainContainer, {backgroundColor: colors.themeWhite}]}>
      <GreadientHeader
        showLeftIcon={true}
        showRightIcon={false}
